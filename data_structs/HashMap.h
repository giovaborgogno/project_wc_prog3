...
void HashMap<K, T>::put(K clave, T valor)
{
    ...
    if (tabla[pos] != NULL)
    {
        //if(tabla[pos]->getValor()==valor) tabla[pos]->setValor(tabla[pos]->getValor()++);
        //else throw 409;
        throw 409;
    }
...
template<class K, class T>
T HashMap<K, T>::getByPos(unsigned int pos) {
    if(tabla[pos] == NULL)throw 199;
    return tabla[pos]->getValor();

}

template<class K, class T>
unsigned int HashMap<K, T>::getTamanio() {
    return tamanio;
}
